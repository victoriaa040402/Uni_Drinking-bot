...
def playing(player):
    ...
    answer = input("Player "+ str(player+1)+ " enter a "+ str(wordLength) + " letter word beginning with "+ wordStart+".") #+1 so it displays the right player number
    answer.lower()
    if wordStart == answer[0] and len(answer) == wordLength and wordCheck(answer) == True:
        return True
    else:
        return False
